...
const init = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    // Add this new handler for users
...
// In your backend socket.js - add debug logging
    ...
    socket.on("acceptRide", async (data, callback) => {
      ...
      try {
        ...
        if (ride.status === "accepted") {
          ...
          if (typeof callback === "function") {
            callback({ success: false, message: "This ride has already been accepted by another driver." });
          }
        ...
        ride.driverMobile = driver ? driver.phone : "N/A";
        await ride.save();
        ...
        socket.emit("rideAcceptedByDriver", driverData);
        ...
        socket.broadcast.emit("rideAlreadyAccepted", { rideId });

      } catch (error) {
        console.error(`❌ Error accepting ride ${rideId}:`, error);
        if (typeof callback === "function") {
          callback({ success: false, message: "Failed to accept ride" });
        }
      }
    });
...
//       lng: longitude,
//       status: status || "Live",
//       vehicleType: "taxi", // You can get this from driver data
//       timestamp: Date.now()
//     });
...
//       return;
//     }
//     socket.join(userId.toString());  // Ensure string for consistency
//     console.log(`👤 User ${userId} joined room with socket ${socket.id}`);
//   });
